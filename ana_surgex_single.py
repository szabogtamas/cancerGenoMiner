...
from cancerGenoMiner import environment_definiton, par_examples, gdc_features, plotting_tools, survival_tools, xena_tools, gex_tools, reporting_tools
import connectDrive
...
def create_pipeline(*,
    ...
    introSpect.flowNodes.channelNodes(*nodes, main_kws=main_kws, location=location+'/pipeline', generalSettings=general_configs, containerPaths=container_paths, verbose=verbose)
...
class getSurvival(nextflowProcess):
    ...
    def dependencies(self):
        return {
            'imports': ['import connectDrive', 'from typing import Union, Tuple', 'from cancerGenoMiner import par_examples, gdc_features, survival_tools, xena_tools, gex_tools'],
            'inhouse_packages': [cgm_folder, intro_folder, drive_folder]
        }
    ...
    def customize_features(self):
        self.modified_kws = {
            "outFile": (1, "-o", "--outFile", {"dest": "outFile", "help": "Location where results should be saved. If not specified, STDOUT will be used.",},),
            "gd": (2, "--gd", {"dest": "gd", "help": "Temporary file to store gene symbol mapping.",},),
        }
        return None
    ...
    def process(self,
        *,
        ...
        if survival_table in ['None', None]:
            ...
            clinicals = xena_tools.fix_phenotype_factorlevels(clinicals, xena_hub, dataset, 'sample_type.samples', leveldict={"NaN": "NaN"})
...
class plotSurvival(nextflowProcess):
    ...
    def dependencies(self):
        return {
            'imports': ['import scipy', 'import numpy as np', 'from typing import Union, Tuple', 'from cancerGenoMiner import par_examples, gdc_features, plotting_tools, survival_tools, xena_tools, gex_tools'],
    ...
    def channel_pretreat(self):
        return [
            [
                'Channel',
                'from(params.cohorts)',
                "map{['"+'"'+"'+"+'params.report_title.replaceAll("^\\"|\\"\\$", "") + ' + "' (' +" + "it +')"+'"'+"'"+", it+'.md', it]}",
                'set{plotcohorts}',
            ],
        ]
    ...
    def channel_specifications(self):
        return {
            ...
            'plotcohorts': ('tuple', ('note_title', 'note_name', 'plotcohort'), (None, None, 'cohort'), None, False),
            ...
            'plots': ('tuple', ('"${plotcohort}.pgf"', '"${plotcohort}_expression.pgf"'), (None, 'gex'), None, False),
    ...
    def customize_features(self):
        self.modified_kws = {
            "outFile": (1, "-o", "--outFile", {"dest": "outFile", "help": "Location where results should be saved. If not specified, STDOUT will be used.",},),
            "lrt": (2, "--lrt", {"dest": "lrt", "help": "Temporary file to store results of log-rank test.",},),
            "titles": (3, "--titles", {"dest": "titles", "help": "Temporary file to store results of page titles.",},),
            "gex": (4, "--gex", {"dest": "gex", "help": "Distribution of gene expressions.",},),
        }
    ...
    def process(self,
        *,
        ...
        plotrow: int = 5,
        plotcol: int = 4,
        ) -> Tuple[plotting_tools.plt.Axes, list, gex_tools.pd.DataFrame, plotting_tools.plt.Axes]:
        ...
        if cohort in gdc_features.tcga_cohort_diagnosis:
            titles = cohort + '\tImpact of gene expression on survival in ' + gdc_features.tcga_cohort_diagnosis[cohort] + ' ('+cohort+')'
        else:
        ...
        for i in range(len(symbols)):
            ...
            if symbol == gene:
                ...
                symbol = ' ('+symbol+')'
            survival_tools.plotKMpair(clinicals, mask, title=gene+symbol, ax=ax, make_legend=False)
            stat = survival_tools.logRankSurvival(clinicals['time'], clinicals['event'], mask)
        ...
        gex.legend(handles=[pg1, pg2, pg3])
        gex.set_xticklabels([item.get_text() for item in gex.get_xticklabels()], rotation=30, ha='right')
        gex.set_xlabel('')
        gex.set_ylabel('Gene expression (FPKM-UQ)', fontsize=9)
        gex.set_title('Gene expression subset by survival quartiles\n(mild prognosis is 2nd and 3rd quartile)')

        bottom, top = gex.get_ylim()
        top = 0.9*top
        for i, symbol in enumerate(symbols):
            t, p = scipy.stats.ttest_ind(df.loc[(df['prognosis'] == 'poor') & (df['gene'] == symbol), 'gex'].tolist(), df.loc[(df['prognosis'] == 'good') & (df['gene'] == symbol), 'gex'].tolist(), equal_var = False)
            s = 'p={:1.5f}'.format(p)
            if p < 0.05:
                s = '*'
                if p < 0.01:
...
class makeHeatmap(nextflowProcess):
    ...
    def dependencies(self):
        return {
            'imports': ['import numpy as np', 'from typing import Tuple', 'from cancerGenoMiner import survival_tools, reporting_tools'],
            'inhouse_packages': [cgm_folder, intro_folder]
        }
    ...
    def channel_specifications(self):
        return {
            ...
            'heatmap_f':  ('tuple', ('ltx', 'img', 'left', 'bottom'), (None, None, None, None), None, False),
            'heatmap':  ('tuple', ("'heatmap.tex'", "'tree1.pgf'", "'tree2.pgf'"), ('ltx', 'left', 'bottom'), None, False),
    ...
    def customize_features(self):
        self.modified_kws =  {
            p[0]:(i+1, '--'+p[0], {"dest": p[0], "help": p[1],}) for i,p in enumerate([
    ...
    def process(self,
        fn: str,
        ) -> Tuple[str, reporting_tools.sns.matrix.ClusterGrid, survival_tools.plt.Axes, survival_tools.plt.Axes]:
        """
        ...
        ptable, left_tree, bottom_tree = reporting_tools.linkorder_table(ptable, row_linkage, col_linkage)
        latex_heatmap = reporting_tools.latex_heatmap(ptable)
        heatmap_image = reporting_tools.heatmap_image(ptable)
        return latex_heatmap, heatmap_image, left_tree, bottom_tree, ','.join(ptable.index.values.tolist())
...
class getRefs(nextflowProcess):
    ...
    def dependencies(self):
        return {
            'imports': ['import os', 'from typing import Union', 'from connectDrive import io'],
...
class pptFromFigures(nextflowProcess):
    ...
    def customize_features(self):
        ...
        self.outputs = [
            'file slide_show into slides'
        ]
        self.command =  'echo "% " $report_title "\\n% " $author_name "\\n% " $slide_head $slide_body' \
        ' > figures.md' + '\n            ' \
...
class compileReport(nextflowProcess):
    ...
    def dependencies(self):
        return {
            'imports': ['import numpy as np', 'import os', 'from typing import Union', 'from cancerGenoMiner import reportex_templates'],
            'inhouse_packages': [cgm_folder, intro_folder]
        }
    ...
    def channel_specifications(self):
        return {
            'heatmap':  ('tuple', ("'heatmap.tex'", "'tree1.pgf'", "'tree2.pgf'"), ('textable', 'left', 'bottom'), None, False),
